	...
	<div>
		<b-table bordered hover small fixed stacked="lg" :fields="fields" :items="items = gottenItems">
			...
			<template #cell(fspd)="data">
				<b-form-select v-model="items[data.index].fspd" :options="optionsFspd" class="form-control"></b-form-select>
			</template>
...
<script>
	...
	export default {
		...
		data() {
			return {
				optionsFspd: [
					{ value: null, text: 'Не контролировалась' },
					{ value: 'controlled', text: 'Контролировалась' }
						],
				fields: [{
						key: 'name',
						label: 'Наименование',
